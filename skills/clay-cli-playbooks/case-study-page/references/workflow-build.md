...
```
clay workflows create --name "case-study probe (safe to delete)"          # -> .id
...
String interpolation inside a reference expression works: `"{{base}}/customers"`, where `base` is
...
```json
{"nodeType":"tool","name":"status /customers","incomingEdges":[{"sourceNode":"<previous node>"}],
 "tools":[{"toolType":"clay_action","actionKey":"check-url","actionPackageId":"<resolved packageId>",
   "inputMappingConfig":{"url":{"type":"reference","expression":"{{base}}/customers"}}}],
...
`"keywords":{"type":"static","value":["customers","case-stud","customer-stor","success-stor","testimonial"]}`.
...
`actionCreditsUsed` counted action executions (10 to 11 per domain for the ten-path probe) while
...
Ten-path probe: 24 to 90 s per domain. Sitemap: 7 to 53 s, timeout at about 110 s. One page fetch
